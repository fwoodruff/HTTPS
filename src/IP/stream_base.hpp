...
public:
    ...
    [[nodiscard]] virtual task<stream_result> write_many(std::vector<std::vector<uint8_t>> bufs, std::optional<milliseconds> timeout) {
        for (auto& buf : bufs) {
            auto res = co_await write(std::move(buf), timeout);
            if (res != stream_result::ok) co_return res;
        }
