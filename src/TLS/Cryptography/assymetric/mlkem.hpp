...
cyclotomic_polynomial sample_poly_CBD(uint8_t k, std::span<const uint8_t> b_array);
...
std::pair<public_encapsulation<Params>, private_decapsulation<Params>> generate_key_pair() {
    ...
    std::array<uint8_t, entropy_length> z;
    randomgen.randgen(d);
    randomgen.randgen(z);
    ...
    std::array<uint8_t, 2 * entropy_length * Params.eta_1> eta_buffer;
    ml_kem_key_gen_internal(Params, d, z, encapsulation_key, decapsulation_key, Ase_buffer, eta_buffer);
